...
class FirestoreTaskService {
  ...
  Future<void> createTask(String title, DateTime dueDate, {bool initialized = false})async{
   
    if(user == null) return;

    final taskRef = _firestore.collection('users').doc(user!.uid).collection('tasks');
    try{
      await taskRef.add({
      "title":title,
      "finished":false,
  ...
  Stream<List<TaskModel>> getTasks(){
    ...
    return _firestore.collection('users').doc(user!.uid).collection('tasks').where("finished",isEqualTo: false).orderBy('dueDate',descending: false).snapshots().map((snapshot){
  ...
  Future<TaskModel> getTaskById(taskId)async{
    final task = await _firestore.collection("users").doc(user!.uid).collection("tasks").doc(taskId).snapshots().first;
    return TaskModel.fromFirestore(task);
  }

  Future<void> editTask(String taskId,String title,DateTime dueDate,bool initialized)async{
    try{
